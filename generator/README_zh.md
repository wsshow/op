...
- **提前终止**：`Stop` 解除生成器与消费者的握手阻塞，并请求生成函数返回。
- **简单迭代**：使用 `Next` 方法获取值，直到生成器完成。
- **资源安全**：生成完成时自动发送结束信号；提前放弃迭代时应调用 `Stop`。
...
**一种可能的输出**（两个 goroutine 的打印顺序不保证固定）：
...
调用 `Stop()` 通知生成器停止。它会解除正在等待的 `Yield.Send`，但不能抢占任意用户代码；生成器函数执行其他阻塞工作时，应检查 `yield.Stopped()` 并及时返回。
