...
`op` 是一个精心设计的 Go 工具包，提供可复用的、泛型优先的通用编程工具。每个包都追求性能与易用性，API 简洁，能自然融入 Go 项目。导入顶层包即可使用全部功能，无需分别管理子包依赖。

## 特性

- **高性能**: 优化实现 — 双端队列 O(1) 操作、环形缓冲区底层、尽可能减少内存分配。
...
基于环形缓冲区的高性能泛型双端队列。所有头尾操作均为 O(1)。缓冲区在 push 时自动扩容，空闲时自动收缩。支持随机访问、旋转、任意位置插入和基于谓词的搜索。
...
受 .NET LINQ 启发的 Go 切片链式查询 API。提供过滤、投影、排序、分组、聚合、集合运算和连接操作。`Linq` 为值类型，多数链式方法返回新副本。
